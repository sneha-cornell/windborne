...
def fetch_weather_for_all_balloon_data(all_balloon_data, api_key):
    """Fetches weather data for each balloon entry."""
    combined_data = []
    ...
    if all_balloon_data:
        for i, entry in enumerate(all_balloon_data):
            ...
            progress_bar.progress((i + 1) / len(all_balloon_data))
...
if page == "Home":
    ...
    if st.button("Run Data Acquisition and Preparation"):
        if not openweathermap_api_key:
            ...
            if all_balloon_data:
                combined_data_with_weather = fetch_weather_for_all_balloon_data(all_balloon_data, openweathermap_api_key)
    ...
    if not st.session_state.df_combined.empty:
        ...
        with st.form("filter_form_viz"):
            ...
            apply_filter_viz = st.form_submit_button("Apply Filter", key="apply_filter_viz")
            clear_filter_viz = st.form_submit_button("Clear Filter", key="clear_filter_viz")
